...
const CERTIFICATIONS = [
  { id: "sa", code: "SA", title: "SAFe Agilist", role: "Leadership", price: 995, earlyBird: 795, duration: "2 Days", seats: 20, available: 7, desc: "Foundation certification for enterprise agile leaders. Understand the Lean-Agile mindset and how to lead an agile transformation at enterprise scale.", outcomes: ["Lead SAFe transformations", "Apply SAFe principles", "Coach Agile teams", "Drive business agility"], audience: "Executives, managers, change agents" },
  { id: "ssm", code: "SSM", title: "SAFe Scrum Master", role: "Scrum Master", price: 895, earlyBird: 695, duration: "2 Days", seats: 20, available: 12, desc: "Become a skilled Scrum Master in a SAFe enterprise environment. Master facilitation, coaching, and servant leadership.", outcomes: ["Facilitate PI Planning", "Coach agile teams", "Remove impediments at scale", "Manage dependencies"], audience: "Scrum Masters, Team Leads, Project Managers" },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 40 }}>
          ...
          <div>
            ...
            {['SAFe Agilist', 'SAFe Scrum Master', 'POPM', 'RTE', 'SPC'].map(c => (
              <span key={c} className="footer-link" onClick={() => setPage('certifications')}>{c}</span>
            ))}
          </div>
          <div>
...
const CertificationsPage = ({ setPage, currency }) => {
              ...
              <div key={cert.id} className={`cert-card ${expanded === cert.id ? 'selected' : ''}`} onClick={() => setExpanded(expanded === cert.id ? null : cert.id)}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button className="btn btn-primary btn-sm" onClick={(e) => { e.stopPropagation(); setPage('register'); }}>
                      Register →
                    </button>
                  </div>
                </div>
...
const RegistrationFlow = ({ currency, toast }) => {
        ...
        <div>
          ...
          <div style={{ marginTop: 24, padding: 20, background: '#F0F9FF', borderRadius: 10, border: '1px solid #BAE6FD', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <input type="checkbox" checked={form.isCorporate} onChange={e => setForm({ ...form, isCorporate: e.target.checked })} />
      ...
      case 3: return (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {[['name', 'Full Name', 'text', 'e.g. Jane Smith'], ['email', 'Work Email', 'email', 'jane@company.com'], ['phone', 'Phone Number', 'tel', '+1 555 000 0000'], ['company', 'Company / Organization', 'text', 'Acme Corp'], ['title', 'Job Title', 'text', 'Scrum Master'], ['country', 'Country', 'text', 'United States']].map(([key, label, type, placeholder]) => (
              <div key={key} className="form-group">
                ...
                <input type={type} className="form-input" placeholder={placeholder} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} />
              </div>
            ))}
          </div>
          <div className="form-group">
            <label className="form-label">Experience Level</label>
            <select className="form-input form-select" value={form.experience} onChange={e => setForm({ ...form, experience: e.target.value })}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="For invoice purposes" value={form.gst} onChange={e => setForm({ ...form, gst: e.target.value })} />
          </div>
          <div className="form-group">
            <label className="form-label">Special Accommodations <span style={{ color: 'var(--slate)', fontWeight: 400 }}>(optional)</span></label>
            <textarea className="form-input" rows={2} placeholder="Accessibility needs, dietary requirements, etc." value={form.accommodation} onChange={e => setForm({ ...form, accommodation: e.target.value })} style={{ resize: 'vertical' }} />
          </div>
            ...
            <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 12, padding: 20, marginBottom: 20 }}>
              ...
              {corpEmails.map((email, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                  <input className="form-input" placeholder={`participant${i + 1}@company.com`} value={email} onChange={e => { const ne = [...corpEmails]; ne[i] = e.target.value; setCorpEmails(ne); }} />
                  {i > 0 && <button className="btn btn-sm" style={{ background: '#FEE2E2', color: 'var(--danger)', padding: '0 12px' }} onClick={() => setCorpEmails(corpEmails.filter((_, j) => j !== i))}>×</button>}
                </div>
              ))}
              <button className="btn btn-sm btn-outline-navy" onClick={() => setCorpEmails([...corpEmails, ''])} style={{ marginTop: 4 }}>+ Add Participant</button>
  ...
  return (
    <>
      ...
      <main style={!isAdmin ? { paddingTop: 0 } : {}}>
        ...
        {page === 'certifications' && <CertificationsPage setPage={setPage} currency={currency} />}
        {page === 'register' && <RegistrationFlow currency={currency} toast={toast} />}
      ...
      {!isAdmin && (
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 90, display: 'flex', gap: 6, background: 'rgba(11,22,41,0.95)', padding: '8px 12px', borderRadius: 30, border: '1px solid rgba(201,168,76,0.2)', backdropFilter: 'blur(8px)' }}>
          {[['home', '🏠', 'Home'], ['certifications', '📋', 'Courses'], ['register', '✍️', 'Enroll'], ['corporate', '🏢', 'Corporate']].map(([id, emoji, label]) => (
